  ...
  it('returns correct row labels for groupBy=both', async () => {
    ...
    vi.mocked(CostExplorerClient).mockImplementation(() => ({
      send: vi.fn().mockResolvedValue({
        ResultsByTime: [{
          Groups: [
            { Keys: ['AWS Lambda', 'env$prod'], Metrics: { UnblendedCost: { Amount: '15.00', Unit: 'USD' } } },
          ],
        }],
      }),
    }))
        ...
        ResultsByTime: [
          {
            ...
            Groups: [
              { Keys: ['AWS Lambda'], Metrics: { UnblendedCost: { Amount: '5.00', Unit: 'USD' } } },
            ],
          },
          {
            TimePeriod: { Start: '2026-01-02' },
            Groups: [
              { Keys: ['AWS Lambda'], Metrics: { UnblendedCost: { Amount: '7.00', Unit: 'USD' } } },
            ],
          },
        ],
